...
namespace nn
{
	// non-monotonic (1) ReLU
	template<unsigned BATCH_SIZE, bool KahanErrorSummation = false, bool StoreRealAccumulator = false> requires (BATCH_SIZE > 0)
	class NNB_nm1ReLUb : public interfaces::NeuronBasicInterface, public interfaces::MaccBackPropogableInterface, public interfaces::LimitedNeuronI, public interfaces::BatchNeuronBasicI {
		...
		inline void SuroCalcActivationFunction(float &accum, std::conditional<StoreRealAccumulator, float, uint8_t>::type &rsacc) const {
			...
			if constexpr (StoreRealAccumulator) {
				...
				if (accum < -max_positive_out) { /* x is negaitive, this is "-x__" part */
					accum = negative_multiplier * (accum + max_positive_out);
		...
		NNB_nm1ReLUb(const NNB_nm1ReLUb &) = delete;
	...
	using NNB_nm1ReLU = NNB_nm1ReLUb<1, false>;
}
