...
static const unsigned LOG_INTERVAL = 25;

static const unsigned BATCH_SIZE = 64;
static const unsigned EPOCHS = 4;
...
static const unsigned LAYER_HIDDEN_SIZE = 128;
...
static inline std::tuple<float, float> MNIST_Train_setup(std::vector<MNIST_entry> &mnist_train, const std::vector<MNIST_entry> &mnist_valid, const std::vector<MNIST_entry> &mnist_test, const std::vector<float> &weights_train, const std::vector<float> &weights_test);

void exp_iReLU_ReLU_mnist_cmp1() {
	std::cout << "exp_iReLU_ReLU_mnist_cmp1" << std::endl;
	...
	std::vector<MNIST_entry> mnist_train_copy(mnist_train);

	auto [relu_acc, relu_f1] = MNIST_Train_setup<nn::NNB_ReLUb<BATCH_SIZE, true>>(mnist_train_copy, mnist_valid, mnist_test, mnist_train_classes_w.GetWeights(), mnist_test_classes_w.GetWeights());
	...
	std::cout << "\n==== RESULTS ====\n";
	std::cout << "Leaky-ReLU:\n";
	std::cout << "- Accuracy: " << relu_acc << "%\n";
	std::cout << "- F1-score: " << relu_f1 << "\n";
	std::cout << "Immortal ReLU:\n";
	std::cout << "- Accuracy: " << irelu_acc << "%\n";
	std::cout << "- F1-score: " << irelu_f1 << "\n";
	std::cout << "Asymptotically, they are almost the same.\n";
	std::cout << "\nPlease, note\n";
	std::cout << "due to the implementation conditions, in multi-thread training mode the result is not 100% deterministic (within reasonable limits).\n";
	std::cout << "Brief explanation: due to the peculiarities of the addition of floating-point numbers, the order of addition of numbers is important (when the exponents are different), ";
	std::cout << "and in a multithreaded system there are no guarantees of the order of addition (in my implementation the atomicity is guaranteed, it is unprofitable to implement order guarantees), ";
	std::cout << "so there, even with an error correction algorithm (Kahan summation), the error accumulates, since the learning is quite long. Thank you for your attention!\n";
	return;
}
...
static inline std::tuple<float, float> MNIST_Train_setup(std::vector<MNIST_entry> &mnist_train, const std::vector<MNIST_entry> &mnist_valid, const std::vector<MNIST_entry> &mnist_test, const std::vector<float> &weights_train, const std::vector<float> &weights_test) {
	...
	std::cout << "Training " << TypeName<nnReLU>::Get() << '\n';
	...
	std::cout << TypeName<nnReLU>::Get() << " accuracy calculating...\n";
