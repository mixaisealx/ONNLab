...
void exp_nm1ilReLU_svsg3() {
	std::cout << "exp_nm1ilReLU_svsg3" << std::endl;

	constexpr float NM1_MAX_VALUE = 4.0f;
	...
	using nnNmReLU = nn::NNB_nm1iReLUb<1, false, true>;
	...
	// Output nm1ReLU layer 
	nn::NeuronHoldingStaticLayer<nnNmReLU> layer_m1lr(11, [&](nnNmReLU *const mem_ptr, unsigned) {
	...
	nn::NeuronHoldingStaticLayer<nn::NNB_Linear> layer_m1out(11, [&](nn::NNB_Linear *const mem_ptr, unsigned) {
		new(mem_ptr)nn::NNB_Linear(-1.0f, NM1_MAX_VALUE);
	});

	// Equvivelent ReLU layer 
	nn::NeuronHoldingStaticLayer<nn::NNB_ilReLU> layer_rleq_inc(11, [&](nn::NNB_ilReLU *const mem_ptr, unsigned) { // Increasing (__/)
		new(mem_ptr)nn::NNB_ilReLU(NM1_MAX_VALUE, 0.1f);
	});
	nn::NeuronHoldingStaticLayer<nn::NNB_ilReLU> layer_rleq_dec(11, [&](nn::NNB_ilReLU *const mem_ptr, unsigned) { // Decreasing (\__)
		new(mem_ptr)nn::NNB_ilReLU(NM1_MAX_VALUE, 0.1f);
	});
	nn::NeuronHoldingStaticLayer<nn::NNB_LinearSlimT<1, false, true, -1.0f, NM1_MAX_VALUE>> layer_smout(11, [&](nn::NNB_LinearSlimT<1, false, true, -1.0f, NM1_MAX_VALUE> *const mem_ptr, unsigned) {
		new(mem_ptr)nn::NNB_LinearSlimT<1, false, true, -1.0f, NM1_MAX_VALUE>;
	});
	...
	// Bias
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimGD>> connections_bias_m1out(&layer_bias, &layer_m1lr, [&](nn::NNB_Connection<OptimGD> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
	...
	// Input to
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimGD>> connections_in_m1lr(&layer_inp, &layer_m1lr, [&](nn::NNB_Connection<OptimGD> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
	...
	nn::Monotonic2FieldsHeuristicsEqExV2 mon2filds(-0.25f);
	nn::Monotonic2FieldsProjectingAccessory projector_rl_m1(&layer_m1lr, &layer_rleq, &mon2filds);
	...
	*/
	...
	nn::LearnGuiderFwBPg learnguider_m1({ &layer_inp, &layer_m1lr, &layer_m1out }, &softmax_calculator);

	std::uniform_int_distribution<unsigned> testselector(0, traindata.size() - 1);

	std::ofstream log_loss("exp_nm1ilReLU_svsg3_loss.txt");

	bool stand_learn = true;

	float loss;
	for (size_t iterations = 0; iterations < 3340; ++iterations) {
		// Select datarow
		...
		if (stand_learn) {
			...
			loss = learnguider_rl.FillupOutsError(perfect_out, 0, true);
			learnguider_rl.DoBackward();
		} else {
			learnguider_m1.DoForward();
			loss = learnguider_m1.FillupOutsError(perfect_out, 0, true);
