...
void exp_nm1ReLU_svsg4() {
	std::cout << "exp_nm1ReLU_svsg4" << std::endl;
	...
	*/
	...
	std::ofstream log_loss("exp_nm1ReLU_svsg4_loss.txt");

	bool stand_learn = true;

	const unsigned MAIN_ITERATIONS = 4000;
	const unsigned FINE_TUNING_ITERATIONS = 665;

	float loss;
	unsigned right_count = 0;
	
	for (size_t iterations = 0; iterations < MAIN_ITERATIONS; ++iterations) {
		// Select datarow
	...
	// Fine tunning
	right_count = 0;
	for (size_t iterations = 0; iterations < FINE_TUNING_ITERATIONS; ++iterations) { // 20 iterations are enough for 91% accuracy, rest iteration to train datarow({5,6}, {1})
		// Select datarow
