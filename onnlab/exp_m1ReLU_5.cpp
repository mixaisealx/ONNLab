...
void exp_m1ReLU_5() {
	std::cout << "exp_m1ReLU_5" << std::endl;
	...
	for (size_t iterations = 0; iterations < 100; ++iterations) {
		...
		if (iterations < 52 && (iterations + 1) % 3 == 0) {
			...
			auto &deviation_curr = out.BatchAnalyzer_GetFeildsActivateMinDistance();
			...
			for (unsigned char idx = 0; idx != 2; ++idx) {
				...
				if (ref.distance > 1e-7f && ref.distance < 0.5f) { // The feild has not met && distance to another feild not so big
					inp[0] = ref.best_canditate->input[0];
					inp[1] = ref.best_canditate->input[1];
					learnguider.DoForward();
					out.FlipCurrentInputToAnotherFeild(); // Moving current input processing to another part of activaiton function
				}
			}
		}
	}
