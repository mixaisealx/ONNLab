...
namespace nn
{
	...
	class NNB_ilReLUb : public interfaces::NeuronBasicInterface, public interfaces::ZeroGradBackPropogableInterface, public interfaces::LimitedNeuronI, public interfaces::BatchNeuronBasicI {
		...
		inline void InnerActivationFunction(float &x) const {
			x = (x > 0.0f ? (x < max_positive_out ? x : max_positive_out) : 0.0f); // f(0) == 0, f(max_positive_out) == max_positive_out
		}

		NNB_ilReLUb(const NNB_ilReLUb &) = delete;
		NNB_ilReLUb &operator=(const NNB_ilReLUb &) = delete;
		public:
			NNB_ilReLUb(float max_positive_out = 1.0f, float hidden_derivative = 0.01f, float hidden_error_backprop_factor = 1.0f, float backprop_overkill_factor = 0.0f, unsigned batch_size = BATCH_SIZE):
			...
			float ActivationFunction(float x) const override {
				return (x > 0.0f ? (x < max_positive_out ? x : max_positive_out) : 0.0f); // f(0) == 0, f(max_positive_out) == max_positive_out
			}

			float ActivationFunctionDerivative(float x) const override {
				return (x > 0.0f && x < max_positive_out ? 1.0f : 0.0f); // df(0) == 0.0f, df(max_positive_out) == 0.0f
			}
			...
			float HiddenActivationFunctionDerivative(float x, float backprop_error) const override {
				if (x > 0.0f) {
					if (x < max_positive_out) {
						...
						return hidden_derivative * (backprop_error > 0.0f ? 1.0f : backprop_overkill_factor); // x >= max_positive_out && backprop_error < 0 -> "increasing" max allowed out (moving to x over-positive field)
					}
				} else {
					return hidden_derivative * (backprop_error < 0.0f ? 1.0f : backprop_overkill_factor); // x <= 0 && backprop_error > 0 -> "zeroing" zero (moving to x-negative field making "much zero")
				}
			}
			...
			float BackPropErrorFactor(float accumulator_value) override {
				return accumulator_value > 0.0f && accumulator_value < max_positive_out ? 1.0f : hidden_error_backprop_factor;
			}
	...
	using NNB_ilReLU = NNB_ilReLUb<1, false>;
}
