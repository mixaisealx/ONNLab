...
void exp_ReLU_svsg2() {
	std::cout << "exp_ReLU_svsg2" << std::endl;
	...
	nn::NeuronHoldingStaticLayer<nn::NNB_ReLUb<BATCH_SIZE>> layer_relu1(6, [&](nn::NNB_ReLUb<BATCH_SIZE> *const mem_ptr, unsigned) {
		new(mem_ptr)nn::NNB_ReLUb<BATCH_SIZE>(0.1f);
	});
	...
	// Connections
	const float LEARNING_RATE = 0.25f;
	...
	*/
	...
	size_t OVERALL_ITERATIONS = 1700;
	...
	for (const auto &sample : traindata) {
		// Update inputs
		std::copy(sample.inputs.begin(), sample.inputs.end(), inputs_store[0].begin());
