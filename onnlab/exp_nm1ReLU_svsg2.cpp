...
void exp_nm1ReLU_svsg2() {
	std::cout << "exp_nm1ReLU_svsg2" << std::endl;
	...
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimGD>> connections_lr1_out(&layer_inp, &layer_out, [&](nn::NNB_Connection<OptimGD> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_Connection<OptimGD>(from, to, &optimizerGD, randistributor(preudorandom));
	});
	...
	*/
	...
	nn::LearnGuiderFwBPg learnguider({ &layer_inp, &layer_out}, &softmax_calculator);

	std::uniform_int_distribution<unsigned> testselector(0, traindata.size() - 1);

	//std::map<const datarow *, std::vector<nn::NNB_nm1ReLU *>> flippers;
	...
	for (size_t iterations = 0; iterations < 250; ++iterations) {
		...
		/*if (iterations < 300 && (iterations + 1) % 3 == 0) {
			...
			for (auto &nrn : layer_out.NeuronsInside()) {
				...
				if (distance > 1e-7f) { // The field has not met && distance to another field not so big
					const datarow *row = reinterpret_cast<const datarow *>(nrn.BatchAnalyzer_GetFieldsActivateMinDistanceDropOnPayload()[ref[0] < ref[1]]);
	...
	/*std::map<const nn::NNB_nm1ReLU *, std::tuple<unsigned, unsigned, float, float, float>> fu_both_neurons;

	auto NonMonotonityStatProc = [&](nn::NNB_nm1ReLU &nrn) {
