...
void exp_stabtest1() {
	std::cout << "exp_stabtest1" << std::endl;

	const unsigned BATCH_SIZE = 8;
	const unsigned SINGLE_CHANNEL = 3;

	const unsigned INPS_COUNT = 105;
	const unsigned KERNEL_SZ = 5;
	...
	// Input vector
	std::vector<std::array<float, INPS_COUNT>> inputs_store(BATCH_SIZE);

	// No batch
	nn::NeuronHoldingStaticLayer<nn::NNB_Input_spyable> layer_inp(INPS_COUNT, [&](nn::NNB_Input_spyable *const mem_ptr, unsigned index) {
		new(mem_ptr)nn::NNB_Input_spyable(&inputs_store[SINGLE_CHANNEL][index]);
	});
	nn::NeuronHoldingStaticLayer<nn::NNB_Storage> layer_inp2(INPS_COUNT - KERNEL_SZ + 1, [&](nn::NNB_Storage *const mem_ptr, unsigned index) {
	...
	// Batch
	nn::NeuronHoldingStaticLayer<nn::NNB_Input_spyableB<BATCH_SIZE>> batch_layer_inp(INPS_COUNT, [&](nn::NNB_Input_spyableB<BATCH_SIZE> *const mem_ptr, unsigned index) {
	...
	nn::NeuronHoldingStaticLayer<nn::NNB_StorageB<BATCH_SIZE>> batch_layer_inp2(INPS_COUNT - KERNEL_SZ + 1, [&](nn::NNB_StorageB<BATCH_SIZE> *const mem_ptr, unsigned index) {
	...
	// No batch
	nn::NNB_ConvolutionEssence1d c1onv1d_frame(&layer_inp, &layer_inp2, KERNEL_SZ);
	nn::NNB_ConvolutionHead<OptimGD> layer_c1onv1d(&c1onv1d_frame, &optimizerGD, [](bool) { return 0.0f; });
	...
	// Batch
	nn::NNB_ConvolutionEssence1d batch_conv1d_frame(&batch_layer_inp, &batch_layer_inp2, KERNEL_SZ);
	nn::NNB_ConvolutionHead<OptimGD> batch_layer_conv1d(&batch_conv1d_frame, &optimizerGD, [](bool) { return 0.0f; });
	...
	nn::LearnGuiderFwBPg learnguider1({ &layer_c1onv1d, &layer_relu1, &layer_out}, &softmax_calculator1);

	nn::errcalc::ErrorCalcSoftMAX softmax_calculator2(OUTS_COUNT);
	nn::LearnGuiderFwBPg learnguider2({ &batch_layer_conv1d,&batch_layer_relu1, &batch_layer_out }, &softmax_calculator2, BATCH_SIZE);
