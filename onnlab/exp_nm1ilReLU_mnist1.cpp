...
void exp_nm1ilReLU_mnist1() {
	...
	// Non-monotonic Hidden layer
	nn::NeuronHoldingStaticLayer<nnNmReLU> layer1_nm_hid(LAYER_HIDDEN_SIZE, [&](nnNmReLU *const mem_ptr, unsigned) {
		new(mem_ptr)nnNmReLU(NM1_MAX_VALUE);
	});

	// Non-monotonic Output layer
	nn::NeuronHoldingStaticLayer<nnNmReLU> layer2_nm_out(10, [&](nnNmReLU *const mem_ptr, unsigned) {
		new(mem_ptr)nnNmReLU(NM1_MAX_VALUE);
	});


	// Equivalent Hidden layer
	nn::NeuronHoldingStaticLayer<nnEqReLU> layer1_eq_hid_inc(LAYER_HIDDEN_SIZE, [&](nnEqReLU *const mem_ptr, unsigned) {
		new(mem_ptr)nnEqReLU(NM1_MAX_VALUE);
	});
	nn::NeuronHoldingStaticLayer<nnEqReLU> layer1_eq_hid_dec(LAYER_HIDDEN_SIZE, [&](nnEqReLU *const mem_ptr, unsigned) {
		new(mem_ptr)nnEqReLU(NM1_MAX_VALUE);
	});
	nn::NNB_LayersAggregator layer1_eq_hid({ &layer1_eq_hid_inc, &layer1_eq_hid_dec });
	// Heading summator
	nn::NeuronHoldingStaticLayer<nnLinSumm> layer15_eq_hid_head(LAYER_HIDDEN_SIZE, [&](nnLinSumm *const mem_ptr, unsigned) {
		new(mem_ptr)nnLinSumm;
	});


	// Equivalent Output layer
	nn::NeuronHoldingStaticLayer<nnEqReLU> layer2_eq_out_inc(10, [&](nnEqReLU *const mem_ptr, unsigned) {
		new(mem_ptr)nnEqReLU(NM1_MAX_VALUE);
	});
	nn::NeuronHoldingStaticLayer<nnEqReLU> layer2_eq_out_dec(10, [&](nnEqReLU *const mem_ptr, unsigned) {
		new(mem_ptr)nnEqReLU(NM1_MAX_VALUE);
	});
	nn::NNB_LayersAggregator layer2_eq_out({ &layer2_eq_out_inc, &layer2_eq_out_dec });
	// Heading summator
	nn::NeuronHoldingStaticLayer<nnLinSumm> layer25_eq_out_head(10, [&](nnLinSumm *const mem_ptr, unsigned) {
		new(mem_ptr)nnLinSumm;
	});
	...
	// Connections of Non-Monotinic
	// Bias
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimAlg>> connections_bias_nmhid(&layer_bias, &layer1_nm_hid, [&](nn::NNB_Connection<OptimAlg> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_Connection<OptimAlg>(from, to, &optimizer, -NM1_MAX_VALUE + randistributor_diviation(preudorandom));
	});
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimAlg>> connections_bias_nmout(&layer_bias, &layer2_nm_out, [&](nn::NNB_Connection<OptimAlg> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_Connection<OptimAlg>(from, to, &optimizer, -NM1_MAX_VALUE + randistributor_diviation(preudorandom));
	});
	// Non-Monotinic FeedForward connections
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimAlg>> connection_inp_nmhid(&layer_inp, &layer1_nm_hid, [&](nn::NNB_Connection<OptimAlg> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_Connection<OptimAlg>(from, to, &optimizer, randistributor(preudorandom));
	});
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimAlg>> connection_nmhid_nmout(&layer1_nm_hid, &layer2_nm_out, [&](nn::NNB_Connection<OptimAlg> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_Connection<OptimAlg>(from, to, &optimizer, randistributor(preudorandom));
	});

	// Connections of Equivalent
	// Bias
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimAlg>> connections_bias_eqhid(&layer_bias, &layer1_eq_hid, [&](nn::NNB_Connection<OptimAlg> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_Connection<OptimAlg>(from, to, &optimizer, randistributor_diviation(preudorandom));
	});
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimAlg>> connections_bias_eqout(&layer_bias, &layer2_eq_out, [&](nn::NNB_Connection<OptimAlg> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_Connection<OptimAlg>(from, to, &optimizer, randistributor_diviation(preudorandom));
	});
	// Equivalent FeedForward connections
	// Input -> Hidden EQ1
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimAlg>> connection_inp_eqhid_inc(&layer_inp, &layer1_eq_hid_inc, [&](nn::NNB_Connection<OptimAlg> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_Connection<OptimAlg>(from, to, &optimizer, randistributor(preudorandom));
	});
	// Input -> Hidden EQ2
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimAlg>> connection_inp_eqhid_dec(&layer_inp, &layer1_eq_hid_dec, [&](nn::NNB_Connection<OptimAlg> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_Connection<OptimAlg>(from, to, &optimizer, -randistributor(preudorandom));
	});
	// (Hidden EQ1 + Hidden EQ2) -> Hidden Head
	nn::SparceLayerStaticConnectomHolder2Mult<nn::NNB_StraightConnection> connections_inp_eqhid_head(&layer1_eq_hid, &layer15_eq_hid_head, [&](nn::NNB_StraightConnection *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_StraightConnection(from, to);
	});
	// Hidden Head -> Out EQ1
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimAlg>> connection_eqhid_out_inc(&layer15_eq_hid_head, &layer2_eq_out_inc, [&](nn::NNB_Connection<OptimAlg> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_Connection<OptimAlg>(from, to, &optimizer, randistributor(preudorandom));
	});
	// Hidden Head -> Out EQ2
	nn::DenseLayerStaticConnectomHolder<nn::NNB_Connection<OptimAlg>> connection_eqhid_out_dec(&layer15_eq_hid_head, &layer2_eq_out_dec, [&](nn::NNB_Connection<OptimAlg> *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_Connection<OptimAlg>(from, to, &optimizer, -randistributor(preudorandom));
	});
	// (Out EQ1 + Out EQ2) -> Out Head
	nn::SparceLayerStaticConnectomHolder2Mult<nn::NNB_StraightConnection> connection_eqhid_out_head(&layer2_eq_out, &layer25_eq_out_head, [&](nn::NNB_StraightConnection *const mem_ptr, nn::interfaces::NBI *from, nn::interfaces::NBI *to) {
		new(mem_ptr)nn::NNB_StraightConnection(from, to);
	});

	// Projecting accessory
	nn::Monotonic2FieldsHeuristicsEqExV2 mon2filds(PROJECTION_THRESHOLD);
	nn::Monotonic2FieldsProjectingAccessory projector_hid(&layer1_nm_hid, &layer1_eq_hid, &mon2filds);
	nn::Monotonic2FieldsProjectingAccessory projector_out(&layer2_nm_out, &layer2_eq_out, &mon2filds);


	nn::LearnGuiderFwBPgThreadAble learnguider_mtr_nm({ &layer1_nm_hid, &layer2_nm_out }, BATCH_SIZE, THREADS_COUNT); // Evaluator & backpropagator for training
	nn::LearnGuiderFwBPgThreadAble learnguider_mtr_equiv({ &layer1_eq_hid, &layer15_eq_hid_head, &layer2_eq_out, &layer25_eq_out_head }, BATCH_SIZE, THREADS_COUNT); // Evaluator & backpropagator for training

	nn::LearnGuiderFwBPg learnguider_sing_nm({ &layer1_nm_hid, &layer2_nm_out }, BATCH_SIZE); // Evaluator for validating & testing
	nn::LearnGuiderFwBPg learnguider_sing_equiv({ &layer1_eq_hid, &layer15_eq_hid_head, &layer2_eq_out, &layer25_eq_out_head }, BATCH_SIZE); // Evaluator for validating & testing
	...
	for (unsigned epoch = 0; epoch != EPOCHS; ++epoch) {
		...
		if (epoch == 1) {
			...
			projector_hid.Perform2to1LossyCompression();
			projector_out.Perform2to1LossyCompression();
